...
fed_consts_all, fed_obj_all, n_clients = [], [], []
for n in [1, 5, 10, 20]:
    my_dict = defaultdict(lambda: {})
    for xx in [0, 2, 3, 4, 5, 6, 7, 8, 9, 10]:
...
# print(x)
    n_clients.extend([n]*len(fed_obj_mean))
    fed_consts_all.extend(fed_constr_mean)
    fed_obj_all.extend(fed_obj_mean)
import seaborn as sns
import matplotlib.pyplot as plt
import pandas as pd

tmp_df = pd.DataFrame({"classification loss": fed_obj_all, "number of clients": n_clients, "loss disparity": fed_consts_all})

print(tmp_df.head())
sns.boxplot(data=tmp_df, x="number of clients", y="classification loss")
